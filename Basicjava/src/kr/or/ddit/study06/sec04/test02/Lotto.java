...
public class Lotto {
	public static void main(String[] args) {
		Lotto l = new Lotto();
		int[][][] bundel = l.lottoBundle(12000);
		l.printBundle(bundel);
		
	}
	
	public void printBundle(int[][][] bundel) {
		for (int i = 0; i < bundel.length; i++) {
			int [][] paper = bundel[i];
			System.out.println("---------------------------------------------");
			for (int j = 0; j < bundel[i].length; j++) {
				int[] lotto = paper[j];
				for (int k = 0; k < bundel[i][j].length; k++) {
					int num = lotto[k];
	...
	public int[] generateLotoo() { 
		...
		sort(lotto);
//		Arrays.sort(lotto);
//		System.out.println(arrayTosrting(lotto));
		return lotto;
	}
	
	public String arrayTosrting(int[] lotto) {
		String result = "[";
		for (int i = 0; i < lotto.length; i++) {
			result += lotto[i];
			if (lotto.length-1 == i) {
